...
app.post('/api/analizar-promesa-pago', (req, res) => {
    try {
        ...
        // Extraer pagos con el nuevo motor robusto
        ...
        } else if (suma_pagos < (valor_exigible - 0.01)) { // Tolerancia de centavos
            is_acceptable = false;
...
function extraerPagosInteligente(input, fecha_ref, total_deuda) {
    ...
    for (const parte of partes) {
        ...
        // 1. Buscar Fecha por Chrono
        const chronoRes = parse(p, fecha_ref.toDate(), { forwardDate: true });
        let fecha_obj = null;
        ...
        if (chronoRes.length > 0) {
            ...
            // Lógica del "Martes" (si es hoy, pasar al siguiente)
            const diasSemana = ['domin', 'lune', 'marte', 'miérco', 'mierco', 'jueve', 'vierne', 'sábado', 'sabado'];
            const esHoyLiteral = diasSemana.some(d => p.includes(d));
            if (esHoyLiteral && fecha_obj.isSame(fecha_ref, 'day') && !p.includes('hoy')) {
        ...
        // 3. Extraer Monto (excluyendo lo que Chrono usó para la fecha)
